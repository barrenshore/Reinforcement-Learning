...
def value_iteration(env, gamma=0.9, max_iterations=10**6, eps=10**-3):
    ...
    for it in range(max_iterations):
        ...
        # V(s) = max a{R(s, a) + rPV(s)} for all s 
        # where R(s, a) = sum s'{P(s, a, s')*R(s, a, s')}
        V = np.max(np.sum(P*R + gamma*P*V, axis = -1), axis = -1)
            
        # termination of value iteration
        if np.abs(np.linalg.norm(V-Vpre, ord=np.inf)) < eps:
            break
    
    # 4. Derive the optimal policy using V(s)
    policy = np.argmax(np.sum(P*R + gamma*P*V, axis = -1), axis = -1)
...
def policy_iteration(env, gamma=0.9, max_iterations=10**6, eps=10**-3):
    ...
    # 1. Initial value function from random policy
    V = np.zeros(num_spaces)
    ...
    for it in range(max_iterations): 
        ...
        while True:
            ...
            # termination of policy evaluation
            if np.linalg.norm(V-Vpre, ord=np.inf) < eps:
                break
            
        # greedy 1-step improvement
