...
writer = SummaryWriter("./tb_record_1/REINFORCE_with_baseline")
...
class Policy(nn.Module):
    ...
    def __init__(self):
        ...
        self.shared_layer_1 = nn.Linear(self.observation_dim, self.hidden_size) # obs_dim -> 128
        self.shared_layer_2 = nn.Linear(self.hidden_size, self.hidden_size) # 128 -> 128
        #self.shared_layer_3 = nn.Linear(self.hidden_size, self.hidden_size) # 128 -> 128
        self.aciton_layer = nn.Linear(self.hidden_size, self.action_dim) # 128 -> action_dim
        self.value_layer = nn.Linear(self.hidden_size, 1) # 128 -> 1
    ...
    def forward(self, state):
        ...
        x = self.shared_layer_1(state) # input state
        x = F.relu(x)
        x = self.shared_layer_2(x)
        x = F.relu(x)
        #x = self.shared_layer_3(x)
        #x = F.relu(x)
        action_prob = self.aciton_layer(x) # output action distribution
        state_value = self.value_layer(x) # output state value
