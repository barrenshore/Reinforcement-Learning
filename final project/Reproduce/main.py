...
EnvName = 'walker2d-medium-expert-v0'

def get_D4RL_data(env):
    # Use d4rl.qlearning_dataset to get dataset contains observations, actions, rewards, terminals and next_observations.
    dataset = d4rl.qlearning_dataset(env)
    return dataset


# Generate replay buffer from d4rl dataset
def collect_d4rl(env, device, args, dataset, n_max):
	...
	datasize = np.size(terminal, 0)

	# Collect (s, a, s', r, terminal)
	for t in range(datasize):
		replay_buffer.add(state[t], action[t], next_state[t], reward[t], terminal[t])
		print(f"current t: {t}")
...
def interact_with_environment(env, state_dim, action_dim, max_action, device, args):
	...
	writer = SummaryWriter(f"./tb_record/{EnvName}/behavioral/seed{args.seed}")
	...
	for t in range(int(args.max_timesteps)):
		...
		if done: 
			...
			writer.add_scalar('Avg Return', eval_policy(policy, args.env, args.seed), t)
...
def train_BCQ(state_dim, action_dim, max_action, device, args):
	...
	# Define a tensorboard writer
	writer = SummaryWriter(f"./tb_record/{EnvName}/BCQ_large_batch/seed{args.seed}")
	
	while training_iters < args.max_timesteps: 
		pol_vals = policy.train(replay_buffer, iterations=int(args.eval_freq), batch_size=args.batch_size)

		evaluations.append(eval_policy(policy, args.env, args.seed))
		np.save(f"./results/BCQ_large_batch_{setting}", evaluations)
		...
		# records
		# writer.add_scalar('Reward', episode_reward, t)
		writer.add_scalar('Avg Return', eval_policy(policy, args.env, args.seed), training_iters)

	# normalize return
	AR = np.load(f"./results/BCQ_{setting}")
	NAR = env.get_normalized_score(sum(AR))
	print(f"Normalize Avg Return: {NAR}")
...
if __name__ == "__main__":
	...
	parser.add_argument("--env", default=EnvName)               # OpenAI gym environment name
	...
	parser.add_argument("--batch_size", default=1000, type=int)      # Mini batch size for networks
	...
	parser.add_argument("--collect_dataset", action="store_true")   # If true, collect dataset
	...
	elif args.collect_dataset:
		dataset = get_D4RL_data(env)
		collect_d4rl(env, device, args, dataset, 1e6)
	else:
